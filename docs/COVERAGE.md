...
| Comments and optimizer hints | Partial | Exact `/*+` and documented whitespace-before-plus `/* +` delimiters retain internal provenance across SELECT, EXPLAIN, WITH, table/alias, JOIN, GROUP BY, CTAS, INSERT/UPDATE/DELETE/MERGE, and COPY. Successful hints canonicalize to `/*+ ... */`; ordinary line/block comments—including empty bodies, parse-hostile prose, GBYTYPE/allowed-name collisions, and unrelated plus signs—remain inert. Every genuine body passes one guaranteed-raise structural boundary, so malformed bodies raise `ParseError` at all four parser levels and strict generation rejects malformed direct/nested trees. The modeled directives now also enforce owner, arity, child shape, and finite domains: argument-free ALLNODES/ENABLE_WITH_CLAUSE_MATERIALIZATION/SYN[TACTIC]_JOIN/VERBATIM; GBYTYPE(HASH/PIPE); JTYPE(H/M/FM); two-value DISTRIB(L/R/B/F/A); nonempty one- to three-part PROJS/SKIP_PROJS; and one valid UTF-8 LABEL value of at most 128 octets per directive. Mixed case canonicalizes, CTAS AS/query labels retain both owners, and unspecified repeats remain lossless. Wrong owners and invalid source/programmatic shapes fail atomically. Well-formed unmodeled directives retain plus identity without being advertised as semantic; Q29 owns their full inventory classification, so the row remains Partial |
