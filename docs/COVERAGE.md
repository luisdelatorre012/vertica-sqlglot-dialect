...
| SELECT `[ AT epoch ]` historical-query prefix | Partial | Analyzer-visible `AtEpochSelect`/`AtEpochUnion`/`AtEpochIntersect`/`AtEpochExcept` roots preserve `EPOCH LATEST`, `EPOCH <integer>`, or `TIME '<timestamp>'` on the complete SELECT/CTE/set-operation query. Direct `traverse_scope`, qualification, optimization, source expansion, and lineage expose the same sources, columns, CTEs, and scopes as the unprefixed query; no `.this` unwrapping is required. The legacy Q06 `AtEpochQuery` wrapper remains loadable and renderable for serialized-tree compatibility but is no longer parser-emitted. Missing/invalid values, non-query tails, and prefix-after-WITH placement fail closed; strict Vertica generation validates prefix and query shapes, while direct and nested PostgreSQL/DuckDB/MySQL/SQLite generation fails atomically on the custom root. Q24 is queued because an AT-prefixed query with both WITH and a parenthesized set branch carrying ORDER BY/LIMIT raises a false CTE-placement error, and a leading comment on an AT+WITH query changes owner after the first regeneration. The structurally unrelated CTAS-only `AtEpochProperty` contract is unchanged |
