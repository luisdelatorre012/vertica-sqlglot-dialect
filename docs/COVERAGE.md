...
| Comments and optimizer hints | Partial | Exact `/*+` provenance is retained internally before shared WITH, table/alias, JOIN, and CTAS extraction, so ordinary line/block comments—including empty bodies, parse-hostile prose, and allowed-name collisions—bypass nested Hint parsing and remain inert. The exact GitHub issue #2 metadata-prefixed WITH/window/two-join/ordered-GROUP query parses through `sqlglot.parse` at every error level, preserves nonempty metadata comments through compact/pretty and serialization cycles, and supports scope traversal, qualification, optimization, and lineage. Existing SELECT, WITH, table, JOIN, EXPLAIN, INSERT/UPDATE/DELETE/MERGE, COPY, GBYTYPE, and directed-query typed contracts remain pinned. Whitespace-before-plus delimiters and malformed-hint atomicity are Q27; directive name/placement/arity/domain closure is Q28, so the row remains Partial |
