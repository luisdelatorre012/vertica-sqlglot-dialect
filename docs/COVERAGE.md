...
| SELECT `[ AT epoch ]` historical-query prefix | Semantic | Analyzer-visible `AtEpochSelect`/`AtEpochUnion`/`AtEpochIntersect`/`AtEpochExcept` roots preserve `EPOCH LATEST`, `EPOCH <integer>`, or `TIME '<timestamp>'` on the complete SELECT/CTE/set-operation query. Direct `traverse_scope`, qualification, optimization, source expansion, and lineage expose the same sources, columns, CTEs, and scopes as the unprefixed query; no `.this` unwrapping is required. The legacy Q06 `AtEpochQuery` wrapper remains loadable and renderable for serialized-tree compatibility but is no longer parser-emitted. Missing/invalid values, non-query tails, and prefix-after-WITH placement fail closed; strict Vertica generation validates prefix and query shapes, while direct and nested PostgreSQL/DuckDB/MySQL/SQLite generation fails atomically on the custom root. The structurally unrelated CTAS-only `AtEpochProperty` contract is unchanged |
