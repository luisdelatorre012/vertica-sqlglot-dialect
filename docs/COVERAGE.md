...
| Comments and optimizer hints | Partial | Exact `/*+` and documented whitespace-before-plus `/* +` delimiters retain internal provenance across SELECT, EXPLAIN, WITH, table/alias, JOIN, GROUP BY, CTAS, INSERT/UPDATE/DELETE/MERGE, and COPY. Successful hints canonicalize to `/*+ ... */`; ordinary line/block comments—including empty bodies, parse-hostile prose, GBYTYPE/allowed-name collisions, and unrelated plus signs—remain inert. Every genuine body passes one guaranteed-raise structural boundary, so empty bodies, unbalanced/truncated directives, empty comma positions, and SQLGlot recovery strings raise `ParseError` at all four parser levels without warnings, raw exceptions, partial ASTs, or statement swallowing. Strict generation rejects malformed direct/nested Hint trees before output, while well-formed unmodeled directives retain their plus identity through dump/load and transforms. The exact GitHub issue #2 fixture and a mixed SELECT/WITH/table/JOIN/GROUP BY hint query preserve round-trip and analysis behavior. Directive name/placement/arity/domain closure is Q28, so the row remains Partial |
