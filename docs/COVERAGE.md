...
| `TIMESERIES` | Partial | Clause and TS null-treatment syntax are semantic; server-only projection/GROUP/HAVING and interval restrictions remain documented negatives. Q20 additionally proved malformed required-component paths can return partial ASTs at `WARN`/`IGNORE`; Q22 owns guaranteed-raise and strict-generation closure |
| Event-series `INTERPOLATE` joins | Partial | Predicate syntax is semantic; join-location and single-predicate semantic restrictions remain. A missing operand can return a partial AST at `WARN`/`IGNORE`; Q22 owns guaranteed-raise and strict-generation closure |
| `MATCH` event patterns | Partial | Partition/order/DEFINE/pattern/row mode and regex text are semantic; duplicate/undefined-event and query-shape validation remain. Malformed required components can return a partial AST at `WARN`/`IGNORE`; Q22 owns guaranteed-raise and strict-generation closure |
