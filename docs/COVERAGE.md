...
| --- | --- | --- |
| Quoted/unquoted identifiers and three-part names | Generic | Core query, projection, and COPY fixtures plus a shared strict CREATE TABLE/INSERT/SELECT INTO/DROP TABLE target contract: one to three nonempty valid-UTF-8 components, schema required with namespace/database, 128-byte per-component limit, documented unquoted start/continuation rules, quoted Unicode/reserved payloads, source-case preservation, and strict programmatic-AST validation. Boundary regressions cover 127/128/129-byte ASCII and multibyte names, unpaired surrogates, empty/missing/four-part shapes, contextual names, and falsey extras; the broader reserved-word collision corpus covers MATCH/TIMESERIES/table-property words plus SELECT-family words (`AT`, `EPOCH`, `TIME`, `LATEST`, `ROLLUP`, `CUBE`, `SETS`, `GROUPING`, `OFFSET`) as column, table, and CTE identifiers |
| Comments and optimizer hints | Partial | Exact `/*+` and documented whitespace-before-plus `/* +` delimiters retain internal provenance across SELECT, EXPLAIN, WITH, table/alias, JOIN, GROUP BY, CTAS, INSERT/UPDATE/DELETE/MERGE, COPY, and UNION ALL. Successful hints canonicalize to `/*+ ... */`; ordinary line/block comments—including empty bodies, parse-hostile prose, GBYTYPE/allowed-name collisions, and unrelated plus signs—remain inert. Every genuine body passes one guaranteed-raise structural boundary, so malformed bodies raise `ParseError` at all four parser levels and strict generation rejects malformed direct/nested trees. The modeled directives enforce owner, arity, child shape, and finite domains: argument-free ALLNODES/ENABLE_WITH_CLAUSE_MATERIALIZATION/SYN[TACTIC]_JOIN/VERBATIM; GBYTYPE(HASH/PIPE); JFMT(F/V); JTYPE(H/M/FM); two-value DISTRIB(L/R/B/F/A); UTYPE(U/M); nonempty one- to three-part PROJS/SKIP_PROJS; and one valid UTF-8 LABEL value of at most 128 octets per directive. Directed `:c`/`:v` annotations are also Semantic. Q29's complete 18-entry inventory classifies SELECT-owned DEPOT_FETCH/ECSMODE/SKIP_STATISTICS and table-owned EARLY_MATERIALIZATION as protected Generic opaque hints: plus identity, statement boundaries, serialization, and canonical output are pinned without claiming directive-domain validation. Q30 gives JOIN-owned JFMT and UNION-ALL-owned UTYPE typed, analyzer-stable owners with exact source placement and foreign-atomicity guards; it also gives mixed CTAS-hint/prose/hinted-WITH comments one stable owner and preserves quoted or unquoted LABEL scalar identity through qualification, optimization, and type annotation. Q31's final source, issue-fixture, positive/negative workload, analysis, strict-AST, foreign, seven-runtime, and clean-wheel gate recertifies this contract; the row remains Partial only for the four protected Generic directives |
...
| Feature | Status | Regression surface and remaining work |
| --- | --- | --- |
| `SELECT`, joins, subqueries, CTEs, set operations | Partial | Canonical SQLGlot AST backed by an official 26.2 example corpus covering SELECT core (`ALL`/`DISTINCT`/`MATCH_COLUMNS`/`FOR UPDATE`), `FROM`/`TABLESAMPLE`/joined-table forms (`INNER`/`LEFT`/`RIGHT`/`FULL [OUTER]`/`NATURAL`/`CROSS`), `WHERE`, `HAVING`, `ORDER BY`, `UNION`/`INTERSECT`/`EXCEPT`/`MINUS`, `WITH` (plain, multiple, subordinate, materialization-hinted, and documented `RECURSIVE`), and WHERE/FROM/HAVING subqueries. CTE bodies are restricted to nonempty side-effect-free SELECT/query-extension and supported set-operation roots; inherited VALUES/bare-FROM forms, SELECT INTO, AT-epoch and executable/admin/DML/DDL roots fail through a guaranteed-raise boundary at every error level. Outer WITH is valid on SELECT and in the documented target-following `INSERT INTO target WITH ... SELECT` form; leading-WITH nonqueries fail atomically. Per-CTE MATERIALIZED/USING KEY and SEARCH/CYCLE are rejected, while strict generation validates With/CTE placement, aliases/columns, recursion state, body types, modifiers, and unknown fields. Joined-table parsing and strict generation own every canonical Join field: ordinary INNER/default and LEFT/RIGHT/FULL `[OUTER]` forms require exactly one ON or USING predicate; CROSS and NATURAL reject both; NATURAL outer joins retain their required OUTER spelling; comma joins, documented `TABLESAMPLE(percent)`, nested/chained joins, and structured JFMT/JTYPE/DISTRIB hints remain supported. ASOF, STRAIGHT_JOIN, invalid side/kind combinations, and foreign match/directed/global/pivot/secondary-relation fields fail atomically. SELECT-owned left SEMI/ANTI with ON and CROSS/OUTER APPLY retain documented architecture-approved EXISTS/LATERAL lowerings; invalid variants fail rather than losing semantics. SELECT qualifiers are closed to omitted/explicit `ALL` and plain `DISTINCT`; `DISTINCT ON`, TOP, foreign kinds/operation modifiers, duplicate qualifiers, unsupported tail permutations, and multiple ORDER/LIMIT/OFFSET/lock clauses fail closed at every error level. `FOR UPDATE [OF table[, ...]]` is the only lock form; foreign strengths/waits fail in parsing and strict generation, and an unparenthesized compound-query lock is promoted from SQLGlot's right-branch placement to the owning set root. Parser-produced OF targets use canonical identifier paths rather than selectable Table nodes, so scope traversal, qualification, optimization, and lineage ignore them as sources across aliases, joins, subqueries/CTEs, compound queries, and AT-prefixed roots while serialization, comments, strict generation, and foreign behavior remain stable. Set operations retain canonical left-associated trees: `UNION` supports default/explicit DISTINCT and `ALL`, with typed UTYPE(U/M) retained on its exact UNION ALL owner; `INTERSECT`, `EXCEPT`, and canonicalized `MINUS` are DISTINCT-only. Their inherited `ALL` forms and every name-matching/correspondence field fail closed in parsing and strict generation, including nested/programmatic trees, without disturbing parentheses, branch-local tails, whole-compound tails, scope traversal, qualification, optimization, or lineage. `GROUP BY` uses the canonical-compatible `VerticaGroup` subclass with one source-ordered list interleaving ordinary expressions, repeated `ROLLUP`, `CUBE`, and `GROUPING SETS` items without rebucketing; material parentheses and empty grouping sets are retained, `GROUPING_ID()` and explicit arguments remain canonical, and the documented `GBYTYPE(HASH|PIPE)` hint is typed. Inherited `GROUP BY ALL`/`DISTINCT`/`TOTALS`/`WITH ROLLUP` forms fail closed at every error level, malformed/programmatic trees are strictly rejected, and the custom node fails atomically in four foreign dialects while qualification, optimization, scope traversal, and lineage preserve it. Q21 closes the installed ordinary query-field inventory before both parser return and PostgreSQL SELECT preprocessing: undocumented WINDOW/CONNECT/LATERAL VIEW/PIVOT, projection-star modifiers, distribution/sort/cluster fields, ORDER SIBLINGS/WITH FILL/explicit NULLS ordering, table ONLY/historical-AT, and foreign TABLESAMPLE methods/ROWS/PERCENT/REPEATABLE options fail at every parser error level and in strict direct/nested generation, including falsey or unknown programmatic fields. Bare numeric `TABLESAMPLE(percent)` remains canonical on tables and named subqueries with range and sampling behavior server-side; QUALIFY, left SEMI/ANTI, and CROSS/OUTER APPLY remain the only approved inherited query lowerings, and generated INNER/LEFT LATERAL JOIN `ON TRUE` output reparses under the same contract. AT-prefixed WITH roots now compose with parenthesized UNION/INTERSECT/EXCEPT branches carrying local ORDER BY/LIMIT tails; the wrapper-only validation keeps the CTE root allowlist closed, and historical-root comments remain text-stable at the `AT` prefix. The Q25 recertification corpus proves the complete Q09–Q24 query contract composes in realistic multi-statement temporary-table pipelines, including public-root scope traversal, qualification, optimization, raw-source lineage, compact/pretty and serialization stability, all-level negative-script atomicity, strict AST rejection, and direct/nested foreign failure |
| SELECT `INTO [TABLE]` targets | Semantic | Typed `SelectInto` root with an `IntoTableClause` target preserves `GLOBAL`/`LOCAL` scope, `TEMP`/`TEMPORARY` spelling, and `ON COMMIT` exactly, and always regenerates the optional `TABLE` keyword; permanent and temporary targets share the strict one-/two-/three-part, schema-with-outer-qualifier, UTF-8, 128-byte, quoted/unquoted, and programmatic-AST identifier contract (namespace/database remain syntactically identical and resolved server-side). The clause is accepted only immediately after the SELECT list, `ON COMMIT` only immediately after a temporary target, and explicit/implicit/quoted aliases, column lists, duplicate or misplaced INTO/ON COMMIT clauses, permanent temporary members, recognized incomplete variants, PostgreSQL `STRICT`/`UNLOGGED`/variable-list forms, and malformed or over-qualified names fail closed at every error level without returning a partial query; WITH, nested, TIMESERIES, parenthesized, and formally admitted set-operation positions retain typed round trips. Foreign generation fails atomically instead of inheriting SQLGlot's silent `SUPPORTS_SELECT_INTO` CTAS rewrite, while foreign-parsed canonical `exp.Into` still renders for the unscoped forms Vertica accepts; Q25 additionally traces a temporary INTO target through CTAS/subordinate and hinted CTEs, an INSERT-populated definition-form temporary table, and the raw source query under ordinary public lineage APIs |
| SELECT `[ AT epoch ]` historical-query prefix | Semantic | Analyzer-visible `AtEpochSelect`/`AtEpochUnion`/`AtEpochIntersect`/`AtEpochExcept` roots preserve `EPOCH LATEST`, `EPOCH <integer>`, or `TIME '<timestamp>'` on the complete SELECT/CTE/set-operation query. Direct `traverse_scope`, qualification, optimization, source expansion, and lineage expose the same sources, columns, CTEs, and scopes as the unprefixed query; no `.this` unwrapping is required. The legacy Q06 `AtEpochQuery` wrapper remains loadable and renderable for serialized-tree compatibility but is no longer parser-emitted. Missing/invalid values, non-query tails, and prefix-after-WITH placement fail closed; strict Vertica generation validates prefix and query shapes, while direct and nested PostgreSQL/DuckDB/MySQL/SQLite generation fails atomically on the custom root. Plain, subordinate, and materialization-hinted WITH clauses compose with parenthesized UNION/INTERSECT/EXCEPT branches carrying branch-local and whole-compound ORDER BY/LIMIT tails at every parser error level. Historical-root comments have one stable typed owner and render at the `AT` prefix in compact and pretty modes. The structurally unrelated CTAS-only `AtEpochProperty` contract is unchanged |
...
| `TIMESERIES` | Partial | Clause and TS null-treatment syntax are semantic; malformed slice names/quoted intervals, partition lists, required ORDER BY, parentheses, duplicate clauses, and non-SELECT placement fail through a TIMESERIES-specific guaranteed-raise boundary at all four parser levels. Strict generation validates `TimeseriesSelect`, `Timeseries`, and synthetic `TimeseriesSlice` fields before emission; malformed direct/nested trees fail atomically and valid trees retain comments, serialization, optimizer/type behavior, and four-dialect foreign failure. Server-only projection/GROUP/HAVING and interval restrictions remain documented negatives |
| Event-series `INTERPOLATE` joins | Partial | Predicate syntax is semantic; missing operands, invalid directions, and incomplete VALUE forms fail through an INTERPOLATE-specific guaranteed-raise boundary at all four parser levels. Strict generation requires two expression operands plus a typed PREVIOUS/NEXT marker and rejects direct/nested malformed trees atomically; comments, Boolean type metadata, and four-dialect foreign failure remain stable. Join-location and single-predicate semantic restrictions remain server-side |
| `MATCH` event patterns | Partial | Partition/order/DEFINE/pattern/row mode and regex text are semantic; missing or malformed required components, parentheses, and duplicate clauses fail through a MATCH-specific guaranteed-raise boundary at all four parser levels. Strict generation validates every typed MATCH/MatchDefinition field, including nonempty ORDER BY/DEFINE/pattern children and the finite row mode, before direct or nested emission; serialization, comments, analysis, and four-dialect foreign failure remain stable. Duplicate/undefined-event and query-shape validation remain server-side |
| Partitioned `LIMIT … OVER` | Semantic | Optimizer-visible `exp.Limit` subclass; `PARTITION BY` and `ORDER BY` required |
| Ordinary `LIMIT`, `OFFSET`, and `FETCH` | Partial | Canonical `Limit`/`Offset` accept nonnegative integer literals (without bounded integer conversion) and anonymous JDBC placeholders; `LIMIT ALL` deliberately canonicalizes to clause absence while preserving following tails. Both official-source relative LIMIT/OFFSET orders are accepted and canonicalize to `LIMIT … OFFSET …`; ORDER must precede either and FOR UPDATE follows. FETCH, comma-form LIMIT, PERCENT/ROW(S)/WITH TIES/BY options, negative/decimal/string/expression/named-placeholder counts, duplicates, and malformed/programmatic fields fail atomically. `FOR UPDATE [OF ...]` remains canonical and analyzer-safe; OF targets are identifier paths that do not enter the selected-source map. The independent partitioned `LIMIT … OVER` contract remains positive-integer-only with required partition and order children |
| `INSERT` | Semantic | Canonical `exp.Insert`; mandatory `INTO`, a strict one-/two-/three-part table target with shared UTF-8/128-byte/quoted-unquoted validation, column targets, `DEFAULT VALUES`, parenthesized nonempty multi-row `VALUES`, query and target-following `WITH` sources, and LABEL hints are validated. Each LABEL directive has exactly one valid unquoted/quoted value of at most 128 UTF-8 octets; unspecified repeated directives remain lossless, while wrong-owner/name/arity/domain forms fail at every parser error level and in strict generation. The complete inherited parse runs fail-closed and every structural validation error routes through an INSERT-specific guaranteed-raise path, so malformed qualification/identifier shapes, missing/conflicting sources, empty or trailing-comma lists, target aliases/partitions, RETURNING/ON CONFLICT and other foreign clauses, unexpected tails, and malformed multi-statement input raise `ParseError` at IMMEDIATE, RAISE, WARN, and IGNORE instead of inventing `INTO`, normalizing syntax, returning a blank-rendering partial AST, or silently dropping a tail; strict generation validates the same target and statement structure before emission |
| `UPDATE` | Semantic | Canonical `exp.Update`; aliases, column aliases, `DEFAULT`, joins, `FROM`, predicates, and source-valid LABEL directives are preserved; LABEL value/UTF-8 length and wrong-owner contracts fail at every parser level and in strict generation; SET subqueries and foreign tail clauses are rejected; `FROM DEFAULT … JOIN` uses a non-table relation leaf |
| `DELETE` | Semantic | Canonical `exp.Delete`; source-valid LABEL directives and subquery predicates are preserved, with LABEL value/UTF-8 length and wrong-owner contracts enforced in parsing and strict generation, while aliases, joined/`USING` targets, leading `WITH`, `RETURNING`, ordering, and limits are rejected |
| `MERGE` | Semantic | Canonical `exp.Merge` plus the existing hinted subtype; required clauses, table/subquery sources, aliases, branch cardinality/actions, pre-`THEN` and trailing `WHERE` filters, unqualified target columns, and source-valid LABEL directives are validated, including all-level/strict owner, value, and UTF-8-length boundaries |
...
| Definition-form `CREATE TABLE` | Semantic | Encoding/access rank, physical order, segmentation, K-safety, partition grouping/count, inherited privileges, quota; targets use the shared strict one-/two-/three-part, schema-with-outer-qualifier, UTF-8, 128-byte, quoted/unquoted, and programmatic-AST identifier contract; the complete table parse is an atomic transaction at every parser error level, so malformed target, physical-design, modifier, list, and end-of-input forms cannot normalize, truncate, fall back to `Command`, or reach raw Python failures. Strict generation classifies and validates the complete canonical root before property sorting or text emission, rejecting malformed definition shapes, containers, typed children, duplicates, mutual exclusions, scope/temporary state, and foreign CREATE fields atomically; valid property-order canonicalization does not mutate the tree. Embedded properties fail atomically in PostgreSQL/DuckDB/MySQL/SQLite generation instead of raising raw `KeyError` |
| `CREATE TABLE AS`, `LIKE`, temporary tables | Semantic | CTAS hints/snapshots, column lists and encodings, segmentation/quota, projection-copy options, scope (including scoped temporary CTAS), commit behavior, and `NO PROJECTION`; AS-clause LABEL hints require one valid value of at most 128 UTF-8 octets per directive, compose losslessly with a SELECT-owned LABEL, and preserve both owners/source order while first-label precedence remains server-side. Ordinary prose between an AS-clause LABEL and a hinted WITH query has one stable `CtasHintProperty` owner and emits exactly once in compact and pretty round trips. Permanent and temporary definition/LIKE/CTAS targets share the strict one-/two-/three-part, UTF-8/128-byte, quoted/unquoted, and programmatic-AST identifier contract. The same all-level atomic parser transaction covers definition, LIKE, and CTAS helpers, while malformed target qualification, duplicate/contradictory GLOBAL/LOCAL/TEMPORARY prefixes, and trailing table/CTAS-list commas fail closed before generic fallback. A whole-tree generator preflight owns the exact definition/LIKE/CTAS property matrix, including LOCAL quota, temporary CTAS segmentation, CTAS `NO PROJECTION`, definition-form historical snapshots, column-list/`ENCODED BY`, wrong node types, duplicates, falsey unknown fields, and nested mutations; all fail with `UnsupportedError` at `RAISE` before property location/rendering. Embedded properties retain Q05's atomic PostgreSQL/DuckDB/MySQL/SQLite failure contract; the CTAS-only `AT EPOCH`/`AT TIME` historical-snapshot value grammar (`AtEpochProperty`) fails closed with `ParseError` at every error level on malformed source values; Q25 exercises scoped and unscoped CTAS, definition-form temporary tables, INSERT population, nested/hinted/recursive CTEs, and downstream SELECT INTO analysis as one recertified lifecycle |
| `DROP TABLE` | Semantic | Ordered multi-target lists share prefix `IF EXISTS` and one trailing `CASCADE` through an atomic root while single targets stay canonical `exp.Drop`; every target shares CREATE/INSERT/INTO's exact one-/two-/three-part, schema-with-outer-qualifier, UTF-8, 128-byte, quoted/unquoted, and strict programmatic-AST identifier contract; undocumented `RESTRICT`, `PURGE`, `TEMPORARY`, `MATERIALIZED`, and `ICEBERG` modifiers fail closed in parsing and generation; catalog existence, dependency effects, and temporary-table auto-drop semantics remain server concerns; Q25 exercises ordered multi-target cleanup, with and without `IF EXISTS`, as the closing statement of recertified multi-statement analysis pipelines |
...
| `COPY` target columns and options | Semantic | Filler columns, transforms, per-column and `COLUMN OPTION` parameters, including duplicate/order/conflict validation; modeled statement hints are LABEL-only, with exactly one valid value of at most 128 UTF-8 octets per directive, lossless unspecified repeats, all-level wrong-owner/domain rejection, and strict AST validation |
