...
| `INSERT` | Semantic | Canonical `exp.Insert`; mandatory `INTO`, table/column targets, `DEFAULT VALUES`, parenthesized nonempty multi-row `VALUES`, query and target-following `WITH` sources, and one LABEL hint are validated. The complete inherited parse runs fail-closed and every structural validation error routes through an INSERT-specific guaranteed-raise path, so missing/conflicting sources, empty or trailing-comma lists, target aliases/partitions, RETURNING/ON CONFLICT and other foreign clauses, unexpected tails, and malformed multi-statement input raise `ParseError` at IMMEDIATE, RAISE, WARN, and IGNORE instead of inventing `INTO`, normalizing syntax, returning a blank-rendering partial AST, or silently dropping a tail; strict generation retains the same shared structural contract |
